...
// implying live analysis that isn't happening. Messages live in
// uiCopy.js's BLUEPRINT_UI[lang].loading.messages (PTM Spanish-parity pass).
...
export default function BlueprintLoading({ onComplete, lang = "en" }) {
  const prefersReducedMotion = useCinematicMotion();
  const [messageIndex, setMessageIndex] = useState(0);
  const [skipVisible, setSkipVisible] = useState(false);
  const ui = BLUEPRINT_UI[lang].loading;
  ...
  }, [messageIndex, onComplete, prefersReducedMotion, messages.length]);
  ...
  const progressPercent = ((messageIndex + 1) / messages.length) * 100;
  ...
  return (
    <div className="mx-auto flex min-h-[50vh] max-w-3xl flex-col items-center justify-center bg-[#0b0b0a] px-6 py-16 text-center text-white">
      <p className="mb-6 text-xs uppercase tracking-[0.4em] text-white/40">
        {BLUEPRINT_UI[lang].intro.eyebrow}
      </p>
