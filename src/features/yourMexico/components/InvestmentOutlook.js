...
export default function InvestmentOutlook({ city }) {
  const outlook = city.investmentOutlook;
  if (!outlook) return null;

  return (
    <CitySection eyebrow="Investment Outlook" title={`Growth & long-term suitability in ${city.name}`}>
      <div className="border border-zinc-200 bg-white p-8 sm:p-10">
        ...
        <p className="mt-6 border-t border-zinc-200 pt-6 text-xs leading-relaxed text-zinc-400">
          This is not financial, investment, or real estate advice, and not a prediction of returns.
          Path To Mexico does not recommend speculative or short-term buying. Anyone considering a
          purchase should verify current conditions independently and consult qualified local
          professionals.
        </p>
      </div>
    </CitySection>
