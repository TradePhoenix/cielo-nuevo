...
const TEXT = {
  en: { eyebrow: "PTM Score", title: (name) => `${name}'s editorial fit assessment` },
  es: { eyebrow: "Puntuación PTM", title: (name) => `Evaluación editorial de idoneidad de ${name}` },
};
...
export default function PTMScoreCard({ city, lang = "en" }) {
  ...
  const tierLabel = lang === "es" ? computed.tierEs : computed.tier;
    ...
    <CitySection eyebrow={t.eyebrow} title={t.title(city.name)}>
      <div className="border border-zinc-200 bg-white p-8 sm:p-10">
        <div className="flex flex-wrap items-baseline gap-4">
          ...
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500">{tierLabel}</p>
        </div>

        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          {PTM_SCORE_FACTORS.map(({ key, label, labelEs }) => {
            const factor = ptmScore.factors[key];
            if (!factor) return null;
            return (
              <div key={key} className="border border-zinc-200 p-5">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500">{lang === "es" ? labelEs : label}</p>
